...
function registerUser(){
	...
	if($email && fetchOne($pdo,'SELECT id FROM usuarios WHERE email=?',[$email])) jsonResponse(['status'=>'error','message'=>'Email ya registrado'],409);
	$hash = password_hash($pass, PASSWORD_DEFAULT);
	$id = insertAndGetId($pdo,'INSERT INTO usuarios(usuario,password_hash,email,bio,foto,activo,fecha_registro) VALUES(?,?,?,?,?,1,NOW())',[$user,$hash,$email,'Nuevo usuario de Pourfect','avatar.png']);
...
function getProfile(){
	...
	$pdo = getDBConnection();
	$user = fetchOne($pdo,'SELECT id,usuario,email,foto,bio,fecha_registro FROM usuarios WHERE id=?',[currentUserId()]);
...
function updateProfile(){
	...
	executeQuery($pdo,'UPDATE usuarios SET usuario = COALESCE(NULLIF(?,""), usuario), bio = ?, email = COALESCE(NULLIF(?,""), email) WHERE id=?',[$usuario,$bio,$email,currentUserId()]);
	jsonResponse(['status'=>'success']);
}
?>
