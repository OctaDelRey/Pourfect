...
function getFavorites($pdo){
	...
	jsonResponse(['status'=>'success','favorites'=>$rows]);
}
...
function getDetail($pdo){
	$id = intval($_GET['id'] ?? 0);
	$row = fetchOne($pdo,'SELECT id,nombre,descripcion,tipo,imagen FROM tragos WHERE id=? AND eliminado=0',[$id]);
	if(!$row) jsonResponse(['status'=>'error','message'=>'No encontrado'],404);
	
	// Obtener calificación del usuario actual si está autenticado
	$userRating = 0;
	if(isAuthenticated()){
		$ratingRow = fetchOne($pdo,'SELECT calificacion FROM calificaciones WHERE usuario_id=? AND trago_id=?',[currentUserId(),$id]);
		if($ratingRow){
			$userRating = intval($ratingRow['calificacion']);
		}
	}
