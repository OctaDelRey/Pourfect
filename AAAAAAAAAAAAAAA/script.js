...
function normalizarTragoAPI(trago, fallbackId = 0) {
  ...
  return {
    ...
    user_rating: typeof trago.user_rating !== 'undefined' ? Number(trago.user_rating) : 0,
    is_favorite: typeof trago.is_favorite !== 'undefined' ? !!trago.is_favorite : false
  };
}
async function obtenerDetalleTrago(drinkId) {
  if (!drinkId) return null;
  try {
    const resp = await fetch(`./api/drinks.php?action=detail&id=${drinkId}`);
    const data = await resp.json();
    if (!resp.ok || data.status !== 'success' || !data.drink) {
      return null;
    }
    return normalizarTragoAPI(data.drink, drinkId);
  } catch (error) {
    console.error('Error obteniendo detalle del trago:', error);
    return null;
  }
}

function actualizarCatalogoTrago(actualizado) {
  if (!actualizado || !catalogoTragos.length) return;
  const idx = catalogoTragos.findIndex(t => t.id === actualizado.id);
  if (idx >= 0) {
    catalogoTragos[idx] = { ...catalogoTragos[idx], ...actualizado };
  }
}
...
async function cargarTragosAPI() {
  try {
    if (typeof loadDrinksFromAPI !== 'function') {
      throw new Error('API de tragos no disponible');
    }

    const baseAPI = await loadDrinksFromAPI();
    const normalizados = Array.isArray(baseAPI)
      ? baseAPI.map((t, idx) => normalizarTragoAPI(t, idx + 1))
      : [];

    catalogoTragos = normalizados;
    renderTragos(normalizados);
  } catch (error) {
    console.error('Error cargando tragos:', error);
    catalogoTragos = [];
    const cont = document.getElementById('lista-tragos');
    if (cont) {
      cont.innerHTML = "<p style='text-align: center; color: #c00; padding: 20px;'>No pudimos cargar los tragos desde la base de datos.</p>";
    }
  }
}
...
async function toggleFavoritoAPI(drinkId, drinkName) {
  try {
    if (typeof toggleFavoriteAPI !== 'function' || !drinkId) {
      throw new Error('API de favoritos no disponible');
    }
    const estado = await toggleFavoriteAPI(drinkId, drinkName);
    await cargarTragosAPI();
    await cargarFavoritosAPI();
    return estado;
  } catch (error) {
    console.error('Error toggleando favorito:', error);
    alert('No pudimos actualizar tus favoritos. Intentalo más tarde.');
    return false;
  }
}
...
async function calificarTragoAPI(trago, rating) {
  if (!trago || !trago.id) {
    alert('Trago no válido para calificar.');
    return false;
  }

  try {
    if (typeof rateDrinkAPI !== 'function') {
      throw new Error('API de calificaciones no disponible');
    }

    await rateDrinkAPI(trago.id, rating);
    const detalleActualizado = await obtenerDetalleTrago(trago.id);
    if (detalleActualizado) {
      Object.assign(trago, detalleActualizado);
      actualizarCatalogoTrago(detalleActualizado);
    }
    await cargarTragosAPI();
    return true;
  } catch (error) {
    console.error('Error calificando trago:', error);
    alert('No pudimos registrar tu calificación. Intentalo más tarde.');
    return false;
  }
}
...
async function cargarFavoritosAPI() {
  try {
    ...
    const contenedor = document.getElementById('perfilFavoritos');
    if (contenedor) {
      contenedor.innerHTML = '<p style="text-align: center; color: #c00; padding: 20px;">No pudimos cargar tus favoritos.</p>';
    }
  }
}
