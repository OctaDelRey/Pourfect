...
function normalizarTragoLocal(trago, idx = 0) {
  return {
    ...
    promedio: trago.promedio || trago.promedio_calificacion || 0,
    total_calificaciones: trago.total_calificaciones || 0,
...
// Modal detalle trago
// --------------------
async function openModal(trago) {
  ...
  // Cargar detalle completo desde API si tiene ID válido
  let tragoCompleto = trago;
  if (trago.id && trago.id > 0 && typeof api !== 'undefined' && api.mode === 'api') {
    try {
      const response = await api.getDrinkDetail(trago.id);
      if (response.status === 'success' && response.drink) {
        tragoCompleto = { 
          ...trago, 
          ...response.drink, 
          user_rating: response.drink.user_rating || 0,
          // Mantener promedio y total_calificaciones del trago original si existen
          promedio: trago.promedio || trago.promedio_calificacion || 0,
          total_calificaciones: trago.total_calificaciones || 0
        };
      }
    } catch (error) {
      console.error('Error cargando detalle del trago:', error);
    }
  }

  const ul = document.getElementById("modalIngredientes");
  ul.innerHTML = "";
  const ingredientes = Array.isArray(tragoCompleto.ingredientes) ? tragoCompleto.ingredientes : [];
  ...
  ol.innerHTML = "";
  const pasos = Array.isArray(tragoCompleto.pasos) ? tragoCompleto.pasos : [];
  if (pasos.length) {
    pasos.forEach(p => {
      const li = document.createElement("li");
      li.textContent = p;
...
function renderEstrellas(trago) {
  ...
  const promedio = trago.promedio || trago.promedio_calificacion || 0;
  ...
  // Estrellas para calificar - usar calificación del usuario si está disponible
  const wrap = document.createElement("div");
  wrap.className = "estrellas";
  wrap.innerHTML = "<small>Tu calificación:</small>";
  
  // Obtener calificación del usuario (desde user_rating o desde el trago)
  let miRating = trago.user_rating || 0;
  
  // Función para renderizar estrellas con la calificación actual
  const renderStars = (currentRating) => {
    wrap.innerHTML = "<small>Tu calificación:</small>";
    for (let i = 1; i <= 5; i++) {
      const star = document.createElement("span");
      star.className = "estrella" + (i <= currentRating ? " activa" : "");
      star.textContent = "★";
      star.onclick = async () => {
        // Intentar guardar en API
        const success = await calificarTragoAPI(trago.id || 0, i);
        if (success) {
          // Actualizar calificación local
          trago.user_rating = i;
          // Re-renderizar estrellas con nueva calificación
          renderStars(i);
          // Recargar datos del trago para actualizar promedio
          if (trago.id && trago.id > 0 && typeof api !== 'undefined' && api.mode === 'api') {
            try {
              // Recargar desde drinks_data para obtener promedio actualizado
              const drinksResponse = await api.getDrinks('', '');
              if (drinksResponse.success && drinksResponse.data) {
                const tragoActualizado = drinksResponse.data.find(t => t.id === trago.id);
                if (tragoActualizado) {
                  trago.promedio = tragoActualizado.promedio || 0;
                  trago.total_calificaciones = tragoActualizado.total_calificaciones || 0;
                  // Actualizar el promedio en la pantalla
                  const infoDiv = cont.querySelector('div');
                  if (infoDiv) {
                    infoDiv.innerHTML = `
                      <p style="margin: 5px 0; color: #666;">
                        ⭐ Promedio: ${trago.promedio.toFixed(1)}/5 (${trago.total_calificaciones} calificaciones)
                      </p>
                    `;
                  }
                }
...
function filtrarTragos() {
  ...
  const fuente = catalogoTragos.length
    ? catalogoTragos
    : (typeof tragos !== 'undefined' ? tragos.map((t, idx) => normalizarTragoLocal(t, idx + 100)) : []);
...
async function cargarCreacionesAPI() {
  try {
    ...
    const normalizadasAPI = Array.isArray(apiCreaciones) ? apiCreaciones.map((t, idx) => normalizarTragoAPI(t, idx + 1)) : [];
    const locales = getUserCreations().map((t, idx) => normalizarTragoLocal(t, idx + 5000));
    ...
    normalizadasAPI.forEach(insertar);
    locales.forEach(insertar);

    renderTragosConEliminar(fusion, 'perfilCreaciones');
  } catch (error) {
  ...
  try {
    ...
    const baseLocales = typeof tragos !== 'undefined'
      ? tragos.map((t, idx) => {
          ...
          // No agregar valores aleatorios, usar los valores reales o 0
          return normalizado;
        })
      : [];
...
async function toggleFavoritoAPI(drinkId, drinkName) {
  ...
  if (!id || id <= 0) {
    console.error('ID de trago inválido:', drinkId);
    alert('Error: No se puede agregar este trago a favoritos. El trago no tiene un ID válido.');
    return false;
  }
  ...
  try {
    ...
    if (typeof toggleFavoriteAPI === 'function') {
      const isFavorite = await toggleFavoriteAPI(id, drinkName);
      if (isFavorite !== false) {
        // Actualizar el estado visual inmediatamente sin recargar toda la lista
        const tragoEnCatalogo = catalogoTragos.find(t => t.id === id);
        if (tragoEnCatalogo) {
          tragoEnCatalogo.is_favorite = isFavorite;
        }
        // Actualizar el botón de favorito en la vista actual
        // Buscar todos los botones de favorito y actualizar el que corresponde
        const botones = document.querySelectorAll('button.btn-fav');
        botones.forEach(btn => {
          const onclickAttr = btn.getAttribute('onclick') || '';
          if (onclickAttr.includes(`toggleFavoritoAPI(${id},`)) {
            btn.innerHTML = isFavorite ? "❤️" : "🤍";
            btn.title = isFavorite ? 'Quitar de favoritos' : 'Agregar a favoritos';
            // Actualizar el atributo onclick para reflejar el nuevo estado
            const nombreTrago = drinkName.replace(/"/g, '\\"');
            btn.setAttribute('onclick', `toggleFavoritoAPI(${id}, "${nombreTrago}")`);
          }
        });
        return isFavorite;
      } else {
    ...
    alert('Error al guardar favorito: ' + (error.message || 'Error desconocido'));
    return false;
  }
}
...
async function calificarTragoAPI(drinkId, rating) {
  const id = parseInt(drinkId);
  if (!id || id <= 0) {
    console.error('ID de trago inválido para calificar:', drinkId);
    return false;
  }
  ...
  try {
    ...
    if (typeof rateDrinkAPI === 'function') {
      const success = await rateDrinkAPI(id, rating);
      if (success) {
        // Recargar el detalle del trago para actualizar promedios
        if (typeof api !== 'undefined' && api.mode === 'api') {
          try {
            const response = await api.getDrinkDetail(id);
            if (response.status === 'success' && response.drink) {
              // Actualizar el trago en el catálogo
              const tragoEnCatalogo = catalogoTragos.find(t => t.id === id);
              if (tragoEnCatalogo) {
                tragoEnCatalogo.user_rating = response.drink.user_rating || rating;
              }
              return true;
            }
          } catch (error) {
  ...
  usuarios.forEach(usuario => {
    ...
    card.innerHTML = `
      ...
      <div class="usuario-stats">
        <span>${usuario.total_tragos || 0} tragos creados</span>
        <span>⭐ ${(usuario.promedio_calificacion || 0).toFixed(1)} promedio</span>
      </div>
      <div class="usuario-tragos">
        <h4>Tragos creados:</h4>
        <div class="tragos-list">
          ${usuario.total_tragos > 0 ? '<span>Haz clic para ver detalles</span>' : '<span>Sin tragos creados</span>'}
        </div>
      </div>
    `;
...
async function verPerfilUsuario(usuario) {
  // Cargar perfil completo desde API si está disponible
  let perfilCompleto = usuario;
  if (usuario.id && typeof getUserProfileAPI === 'function') {
    try {
      const perfilData = await getUserProfileAPI(usuario.id);
      if (perfilData && perfilData.user) {
        perfilCompleto = {
          ...usuario,
          ...perfilData.user,
          drinks: perfilData.drinks || []
        };
      }
    } catch (error) {
  ...
  let fechaRegistro = '2024';
  if (perfilCompleto.fecha_registro) {
    try {
      const fecha = new Date(perfilCompleto.fecha_registro);
  ...
  // Obtener promedio real
  const promedio = perfilCompleto.promedio_calificacion || 0;
  const totalTragos = perfilCompleto.total_tragos || (perfilCompleto.drinks ? perfilCompleto.drinks.length : 0);
  const totalFavoritos = perfilCompleto.total_favoritos || 0;
  ...
  modal.innerHTML = `
    <div class="modal-content" style="max-width: 500px;">
      <div class="modal-header">
        <h2>Perfil de @${perfilCompleto.usuario}</h2>
      ...
      <div class="modal-body">
        <div class="usuario-header" style="margin-bottom: 20px;">
          <img src="${perfilCompleto.foto && perfilCompleto.foto !== 'avatar.png' ? 
            (localStorage.getItem('userImage_' + perfilCompleto.foto) || 'imagenes/' + perfilCompleto.foto) : 
            'imagenes/avatar.png'}" 
            alt="Avatar" class="usuario-avatar" style="width: 80px; height: 80px;" 
            onerror="this.src='imagenes/avatar.png'">
          <div class="usuario-info">
            <h3>@${perfilCompleto.usuario}</h3>
        ...
        <div class="usuario-bio" style="margin-bottom: 20px;">
          ...
          ${perfilCompleto.bio || 'Sin biografía disponible'}
        </div>
        <div class="usuario-stats" style="margin-bottom: 20px;">
          <div style="display: flex; justify-content: space-between;">
            <span>📊 ${totalTragos} tragos creados</span>
            <span>❤️ ${totalFavoritos} favoritos</span>
          </div>
        </div>
        <div class="usuario-tragos">
          <h4>Tragos creados por este usuario:</h4>
          <div class="tragos-list">
            ${perfilCompleto.drinks && perfilCompleto.drinks.length > 0 ? 
              perfilCompleto.drinks.map(trago => 
                `<span class="trago-tag" onclick="buscarTragoPorUsuario('${trago.nombre}', '${perfilCompleto.usuario}')">${trago.nombre}</span>`
              ).join('') : 
              '<span>Sin tragos creados</span>'}
          </div>
        </div>
      </div>
    </div>
  ...
  if (formAgregar) {
    formAgregar.addEventListener("submit", async (e) => {
      ...
      try {
        ...
        const tragoNormalizado = normalizarTragoLocal({ ...nuevoTrago });

        if (window.api && window.api.mode === 'api') {
          const id = await addDrinkToAPI(nuevoTrago);
          if (id) {
            tragoNormalizado.id = id;
          }
          addUserCreation(tragoNormalizado);
          alert("Trago agregado con éxito!");
        } else {
          // Guardar como creación del usuario (modo local)
          addUserCreation(tragoNormalizado);
          alert("Trago agregado con éxito!");
        }
        cargarTragosAPI();
...
async function eliminarTrago(tragoId, tragoNombre) {
  ...
  try {
    ...
    // Eliminar de las creaciones locales (si existiera copia)
    deleteUserCreation(tragoId);
    // Actualizar catálogo en memoria
    catalogoTragos = catalogoTragos.filter(t => (t.id !== tragoId) && (t.nombre || '').toLowerCase() !== (tragoNombre || '').toLowerCase());
