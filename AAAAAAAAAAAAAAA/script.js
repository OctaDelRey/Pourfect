...
function mostrarCreacionesPerfil() {
  // Usar API si está disponible
  if (typeof getUserDrinksAPI === 'function') {
    cargarCreacionesAPI();
  } else {
    // Fallback a método local - mostrar mensaje
    const contenedor = document.getElementById('perfilCreaciones');
    if (contenedor) {
      contenedor.innerHTML = '<p style="text-align: center; color: #666; padding: 20px;">Conecta con la API para ver tus creaciones</p>';
    }
  }
}
...
async function cargarCreacionesAPI() {
  try {
    const creaciones = await getUserDrinksAPI();
...
async function eliminarTrago(tragoId, tragoNombre) {
  ...
  try {
    // Intentar eliminar con API
    if (typeof deleteDrinkAPI === 'function') {
      const eliminado = await deleteDrinkAPI(tragoId);
      if (eliminado) {
        alert('✅ Trago eliminado exitosamente');
        // Recargar las creaciones
        mostrarCreacionesPerfil();
      } else {
        alert('❌ Error al eliminar el trago');
      }
    } else {
