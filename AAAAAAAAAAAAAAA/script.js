  ...
  return {
    ...
    promedio: typeof trago.promedio !== 'undefined'
      ...
      : (typeof trago.promedio_calificacion !== 'undefined'
        ? Number(trago.promedio_calificacion)
        : 0),
...
async function hacerLogin() {
  ...
  try {
    ...
    if (typeof handleLogin === 'function') {
      ...
      try {
        const checkResponse = await api.checkAuth();
        if (!checkResponse.authenticated) {
          console.warn('Advertencia: La sesión no se estableció correctamente después del login');
        }
      } catch (checkError) {
        console.error('Error verificando sesión después del login:', checkError);
      }
  ...
  try {
    ...
    if (typeof loadDrinksFromAPI === 'function') {
      try {
        ...
        console.log('Tragos cargados desde API:', baseAPI?.length || 0);
      } catch (apiError) {
...
async function toggleFavoritoAPI(drinkId, drinkName) {
  try {
    ...
    // Fallback a método local solo si no es error de autenticación
    toggleFavorito(drinkName);
    const fuente = catalogoTragos.length ? catalogoTragos : (typeof tragos !== 'undefined' ? tragos : []);
    renderTragos(fuente);
    return true;
...
// Función helper para obtener el año de una fecha
function obtenerAnioRegistro(fechaRegistro) {
  if (!fechaRegistro) return '—';
  
  // Si es un string en formato MySQL DATETIME (YYYY-MM-DD HH:MM:SS)
  if (typeof fechaRegistro === 'string') {
    // Extraer solo la parte de la fecha (YYYY-MM-DD)
    const fechaParte = fechaRegistro.split(' ')[0];
    if (fechaParte) {
      const anio = fechaParte.split('-')[0];
  ...
  try {
    ...
    console.error('Error parseando fecha:', fechaRegistro, e);
  }
...
function mostrarResultadosUsuarios(usuarios) {
  ...
  usuarios.forEach(usuario => {
    ...
    const anioRegistro = obtenerAnioRegistro(usuario.fecha_registro);
...
async function verPerfilUsuario(usuario) {
  ...
  const favoritos = typeof perfil.total_favoritos === 'number'
    ...
    : (perfil.favoritos ?? '—');
  const anioRegistro = obtenerAnioRegistro(perfil.fecha_registro);
